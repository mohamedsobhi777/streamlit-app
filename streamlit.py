...
hdpunks_activities_fpath = './../punks/ETHEREUM/0x3e86e26915403ae0e1cff7e7b23377b3a30104a0/activities_SELL.json'
covidpunks_activities_fpath = './../punks/ETHEREUM/0xe4cfae3aa41115cb94cff39bb5dbae8bd0ea9d41/activities_SELL.json'
punk_babies_activities_fpath = './../punks/ETHEREUM/0xf2febe0fc74ebea939240851686d5b5455d200ab/activities_SELL.json'
bastard_gan_punks_v2_activities_fpath = './../punks/ETHEREUM/0x31385d3520bced94f77aae104b406994d8f2168c/activities_SELL.json'


# # Read CSV files

cryptophunks_activities = pd.read_json(cryptophunks_activities_fpath)
hdpunks_activities = pd.read_json(hdpunks_activities_fpath)
covidpunks_activities = pd.read_json(covidpunks_activities_fpath)
punk_babies_activities = pd.read_json(punk_babies_activities_fpath)
bastard_gan_punks_v2_activities = pd.read_json(bastard_gan_punks_v2_activities_fpath)

# # Date info
cryptophunks_activities['year-month'] = cryptophunks_activities.date.apply(lambda x: '-'.join(str(x).split('-')[:2]))
hdpunks_activities['year-month'] = hdpunks_activities.date.apply(lambda x: '-'.join(str(x).split('-')[:2]))
covidpunks_activities['year-month'] = covidpunks_activities.date.apply(lambda x: '-'.join(str(x).split('-')[:2]))
punk_babies_activities['year-month'] = punk_babies_activities.date.apply(lambda x: '-'.join(str(x).split('-')[:2]))
bastard_gan_punks_v2_activities['year-month'] = bastard_gan_punks_v2_activities.date.apply(lambda x: '-'.join(str(x).split('-')[:2]))


cryptophunks_activities['token_id'] = cryptophunks_activities.nft.apply(lambda x: int(x['type']['tokenId']))
hdpunks_activities['token_id'] = hdpunks_activities.nft.apply(lambda x: int(x['type']['tokenId']))
covidpunks_activities['token_id'] = covidpunks_activities.nft.apply(lambda x: int(x['type']['tokenId']))
punk_babies_activities['token_id'] = punk_babies_activities.nft.apply(lambda x: int(x['type']['tokenId']))
bastard_gan_punks_v2_activities['token_id'] = bastard_gan_punks_v2_activities.nft.apply(lambda x: int(x['type']['tokenId']))
...
# #Cleaning Crypto Phunks
# cryptophunks_activities_sorted = cryptophunks_activities.sort_values(by='priceUsd', ascending=False).copy()
# cryptophunks_activities_sorted[['buyer', 'seller', 'price', 'priceUsd', 'year-month', 'token_id']].iloc[:10]



# cryptophunks_groups = cryptophunks_activities.groupby('token_id')
# crypto_phunks_bad_tokens = [ x[0] for x in cryptophunks_groups if len(x[1]) >= 10 and max( len(set(x[1]['seller'])), len(set(x[1]['buyer'])) ) <= 10 ]
# cryptophunks_activities['wash_trading'] = cryptophunks_activities['token_id'].apply(lambda x: x in crypto_phunks_bad_tokens)

# st.write(f""" 

# Filtering out the tokens with at least 10 transactions but at most 10 unique buyers/sellers

# For CryptoPunks, {len(crypto_phunks_bad_tokens)} tokens were filtered out, none of the other collections had any suspicious wash-trading activities.
# """)
...
# cryptophunks_clean_dataclass = {
#     'name': 'CryptoPhunks',
#     'data': cryptophunks_activities[ cryptophunks_activities['wash_trading'] == False ][ ['priceUsd', 'year-month']  ],
#     'feature': 'year-month',
#     'mutator': lambda x: len(x)
# }
...
# lhs, rhs = st.columns(2)
# with lhs:
#     plotLineChart([cryptophunks_clean_dataclass], 'Transactions Count')

# with rhs:
#     cryptophunks_dataclass['mutator'] = lambda x: x['priceUsd'].mean()
#     plotLineChart([cryptophunks_clean_dataclass], 'Average Transaction Value (USD)')
# st.write("***")
